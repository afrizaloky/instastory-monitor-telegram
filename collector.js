...
(async () => {
  ...
  const targets = await Promise.all(
    parsedTarget.map(target => ig.user.searchExact(target))
  ); // getting exact user by username
  ...
  targets.forEach(async targetUser => {
    ...
    if (storyItems.length === 0)
      return console.log(`${targetUser.username}'s story is empty`); // we can check items length and find out if the user does have any story to watch
    ...
    Promise.all(
      storyItems
        ...
        .map(r => {
          ...
          const storyElementTarget =
            media_type == 1 ? r.image_versions2.candidates : r.video_versions;
          storyUID.push(r.id);
          storyMetadata.push({
            ...
            send_whatsapp: false, //is story sended - whatsapp
            downloaded: false,
          });
        })
    ).then(() => {
